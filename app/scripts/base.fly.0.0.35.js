 ...
 *
 * ============ 以下将介绍如何定义 fragment 模型  ============
 *
 * 定义一个 Fragment 实例并以指定的 ID 来标识。
 *
 * <pre>
 * e.g:
 *
...
!function($lr/*, undefined*/) {
    ...
    function back(fromUri/* allowUpToHome */) {
        ...
        if ( fromUri && ! _hasBackStackRecords() && _config['home'] ) {
            _navigateUpTo(_config.home);
            return
        }
     ...
     * @private
     */
    var/*function*/ _navigateUpTo = _performNavigateUpTo;/*(up) {
     ...
     * 进行 fragment 切换操作.
     *
     * @param {string} id 指定 fragment 的 id
     * @param {Map} args 参数(optional)
     * @param {boolean} fromUri 是否从 uri 触发(optional)
     * @param {string} animation 切换效果(optional)
     * @private
     */
    function _requestGo/*_go*/(id, args, fromUri, /*fromUser,*//*zIndex, */animation) {
        if ( _hasFragmentTransInProcessing() )
            return;
    ...
    */
     ...
     *
     * @param hash A special hash of Fragment model
     * @returns {boolean}
     * @private
     */
    function _isSpecialRawHash(hash) {
     ...
     * 处理 hash 变更事件.
     *
     * @param oldFragSpec {FragmentSpec}
    ...
    function _handleHashChange(oldFragSpec, /*currently*/newRawHash) {
        ...
        if ( ! _isSpecialRawHash( newRawHash ) )
            return;
    ...
    function _processFragSepcChangedEvent(oldFragSpec, /*currently*/newRawHash) {
        ...
        /* 是否为 page hash */
            ...
            var resolvedNewFragSpec = _resolveFragSpec( newRawHash );

            /*console.log( 'Spec: %s, %s',
                JSON.stringify( oldFragSpec ),
                JSON.stringify( resolvedNewFragSpec ) );*/

            /* 是否 hash 真的需要更新 */
            /* 暂时使用 History API */
            if ( ! _isSameFragmentSpec( oldFragSpec, resolvedNewFragSpec ) ) {
                _triggerGoNext( resolvedNewFragSpec, /* from_user */1, /* from_uri */1 );
